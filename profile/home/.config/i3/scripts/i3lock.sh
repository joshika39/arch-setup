...
#	convert ${images[i]} -fill $MAGENTA2 -colorize 30% ${images[i]}	
	...
	if (( i == 0 )); then
		...
		convert ${images[i]} -gravity center -repage $res1+0+0 -font "$font" -pointsize 26 -fill "$BLUE" -annotate +0+400 "`$text`" ${images[i]}

	else	
		 
		convert ${images[i]} -gravity center -repage $res2+0+0 -font "$font" -pointsize 26 -fill "$BLUE" -annotate +0+0 "`$text`" ${images[i]}
...
--{time,date,layout,verif,wrong}-size=30 \
--time-size=34 \
--date-size=18 \
--verif-text="検証" \
